...
#' @param control This is the control that will be used in the rpart() function. The user has 3 options, one of which is the default selected control by the rpart() function. The remaining two option are: 1) Specity a cp value which will prune each decision tree by the specified value or 2) Specity a two-dimensional data.frame() that contains the variable name(S) as identified in the data component for the first column
#' and the respective cp of each variable in the second column. Variable(s) not included in this data.frame() will use the cp generated by the rpart() function.
...
tree.bins <- function(data, y, bin.nm = "Group.",
  ...
  for (i in 1:(dim(df)[2]-1)) {
    ...
    } else  if (is.null(method) & !is.data.frame(control) & !is.null(control)){
    ...
    } else  if (is.null(method) & is.data.frame(control) & (names(df[,i, drop = F]) %in% as.character(control[,1]))){

      tree = rpart(formula = colnm ~ df[,i], data = df,
                   control = rpart.control(cp = control[control == as.character(names(df[, i, drop = F])), 2]))
      tree.rules = unlist(rpart.lists(tree))
